...
function renderLoraSlots(name, spec, schema) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    h += `<div class="lora-slot-row">` +
      `<input type="text" class="input" data-param="${name}" data-lora-slot="${i}" value="${escAttr(p.path)}" placeholder="LoRA ${i + 1}: full https://….safetensors URL or civitai:MODEL@VERSION" />` +
...
// local files. Picking resolves the key server-side into whatever form this
// provider accepts (MuAPI-hosted URL here), then applies it like an upload.
...
function cloudRender() {
  ...
  files.forEach((o) => {
    ...
    html += '<div class="cloud-card' + (st.selected === o.key ? ' sel' : '') + '" data-key="' + cloudEsc(o.key) + '" data-kind="' + kind + '">' + thumb +
      '<div class="cloud-meta"><div class="truncate" title="' + cloudEsc(o.key) + '">' + cloudEsc(cloudBaseName(o.key)) + '</div><div class="vmeta" style="color:#6b7280">' + cloudFmtSize(o.size) + '</div></div></div>';
  });
...
// Lazy thumbs for the picker: images on scroll-into-view; video posters
// captured to canvas (max 3 at once) so the grid never self-DDoSes.
...
function cloudApplyToUploadModal(url) {
  ...
  st.textContent = 'Cloud file ready ✓';
