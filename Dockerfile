...
#
# MuAPI Prompt Generator — static SPA served by nginx.
...
#
# Build: docker build -t muapi-prompt-generator .
# Run:   docker run --rm -p 8000:80 muapi-prompt-generator
# Open:  http://localhost:8000
#
# Note: generation + catalog routes (/api/generate, /api/models, …) require
# the Worker backend (MUAPI_API_KEY lives server-side), so use
